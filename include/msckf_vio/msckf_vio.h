...
class MsckfVio {
  ...
  private:
    ...
    // Tracking rate
    double tracking_rate;     //跟踪到的旧特征占所有特征的比例

    // Threshold for determine keyframes
    //用于cam状态超出限制时，判断是否删除某cam状态
    double translation_threshold;   //平移阈值
    double rotation_threshold;      //旋转阈值，
    double tracking_rate_threshold; //特征点跟踪率阈值
