...
private:
  ...
  struct ProcessorConfig {
    int grid_row;               //网格的最大行编号
    int grid_col;               //网格的最大列编号
    int grid_min_feature_num;   //网格最小特征数阈值
    int grid_max_feature_num;   //网格最大特征数阈值

    int pyramid_levels;    //最高层金字塔编号
    int patch_size;        //图像金字塔winsize？
    int fast_threshold;    //提取fast角点的阈值
    int max_iteration;     //光流跟踪的最大迭代次数
    double track_precision; //光流跟踪搜索窗口移动必须小于此阈值？
    double ransac_threshold; //ransac的inlier_error阈值
    double stereo_threshold;  //用于极线剔除外点的阈值
  };
   ...
   *    grid index.
   */
  //key为grid序号，value为grid中存储的feature
   ...
   */
  //处理数据
  //输入：raw_vec是原始数组，包含不应被剔除和应被剔除的数据
  //输入：markers标志数据是否应被剔除，为0应被提出，为1不应被剔除
  //输出：refined_vec处理后的数组，只包含不应被剔除的数据
  template <typename T>
  ...
  std::vector<cv::Mat> curr_cam0_pyramid_; //由cv函数生成，存储金字塔图像
  ...
  int before_tracking;   //trackFeatures()之前
  int after_tracking;    //cam0 pre和cur帧间光流跟踪后
  int after_matching;    //curr cam0和cam1 stereoMatch()后
  int after_ransac;      //RANSAC之后
  ...
  //https://www.codeleading.com/article/650575158/
  //message_filters::Subscriber用于时间戳对齐
