...
namespace msckf_vio {
namespace utils {
//从ros获取Isometry3d型4*4的矩阵
//输入：1.node句柄   2.node中的变量名
//输出：Isometry3d型矩阵
...
//从ros获取mat型4*4的矩阵， 传输方式：node
//输入：1.node句柄   2.node中的变量名
//输出：mat型矩阵
cv::Mat getVec16Transform(const ros::NodeHandle &nh,
                          const std::string &field) {
  std::vector<double> v;
  //从ros中获得变量field对应的值（好像ros中的变量）
  nh.getParam(field, v);
  if (v.size() != 16) {
      //https://blog.csdn.net/zzjxiaozi/article/details/6649999
    throw std::runtime_error("invalid vec16!");
  }
  //https://www.cnblogs.com/denny402/p/5035535.html
  cv::Mat T = cv::Mat(v).clone().reshape(1, 4); // one channel 4 rows
  return T;
}
//从ros获取mat型4*4的矩阵 传输方式：XmlRpc
//输入：1.node句柄   2.node中的变量名
//输出：mat型矩阵
